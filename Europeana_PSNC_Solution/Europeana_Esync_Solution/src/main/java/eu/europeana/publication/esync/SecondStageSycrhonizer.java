...
/**
 * 
 * SecondStageSycrhonizer.java
 * Purpose: synchronize documents from source to destination one document by one
 *
...
public class SecondStageSycrhonizer {
	 ...
	 * consume the documents Ids from RabbitMQ receiver with its related operation (insert update delete)
	 * use those Ids to query and return documents from the source collection before synchronizing them to the destination collection
	 * TimeoutException  will accrued only if there is no more data to consume from the RabbitMQ  which mean the data consumed correctly 
	 * 
	 ...
	 *@param reciever
	 *			           RabbitMQ receiver which will be used to consume the synchronized document id with the appropriate operation (insert- update -delete ) 
	 *@return 
	 *                     boolean True when the synchronization and update processes are completed successfully .False otherwise.
	 */ 
	public boolean syncronize(ICollection sourceCollection,
		...
		try {
			...
			QueueingConsumer consumer = new QueueingConsumer(reciever.getChannel());

			reciever.getChannel().basicConsume(reciever.getEndPointName(), true, consumer);
			...
			while (true) {
				try {
					...
					for (String key : map.keySet()) {
						...
						{
							IDocument sourceDocument = sourceCollection.getDocumentById(key);
							destinationCollection.insertDocument(sourceDocument);
							
						} 
						else if (map.get(key).equals("update"))
						{
							IDocument sourceDocument = sourceCollection.getDocumentById(key);
							destinationCollection.updateDocumentUsingId(sourceDocument);
						...
						{
							IDocument sourceDocument = sourceCollection.getDocumentById(key);
							destinationCollection.deleteDocument(sourceDocument);
